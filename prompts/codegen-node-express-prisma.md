...
- **Inferred endpoints** (`inferred_from_signal: "<signal>"`): scaffold the route the same way as a normal endpoint, but add a TODO at the top of the handler: `// TODO: no UI wires up to this endpoint; inferred from <signal>. Add the form or remove the endpoint.` This is the convention for auth endpoints when `auth.json.inferred_from` is `auth_required_screen`, `token_storage_key`, `auth_header`, or `auth_path` — the backend exists; the UI doesn't yet.
