...
> 7. **(Judgment-call fallback.)** None of 1–6 fire AND the inventory shows at least one `forms.standalone_buttons[]` or `forms.forms[]` whose `action: "api_call"` target is **not** a public-form pattern AND the mutated target entity is **not** persisted to `localStorage` in `components.json -> shared_state.storage_keys[]`. → `inferred_from: "judgment:user_owned_mutations"`. **Trust the inventory's `action` classification — do not re-examine the button's current wiring.** The inventory already applies an intent-over-wiring rule (see `prompts/inventory/forms.md`): a `Save`/`Favorite`/`Like`/`Heart` button is `api_call` even when implemented as `useState` or `alert()`, because vibe-coded sites routinely ship with incomplete wiring. If you see `action: "api_call"` you must treat it as a user-owned mutation; do **not** reopen `components/*.tsx` to check whether `fetch()` is actually called, and do **not** "reclassify" the button to `local_state` based on absence of wiring. Signal 7's whole purpose is to commit to a schema-ready posture (placeholder users + nullable `user_id` FKs) when the intent is present but the wiring isn't yet — overriding the inventory defeats the signal. If you genuinely believe the user wants ephemeral local state instead, surface it as an Open Question *in addition to* firing signal 7, not as a replacement for it. Public-form taxonomy (which suppresses signal 7): contact, contact-us, get-in-touch, newsletter, subscribe, subscription, waitlist, wait-list, feedback, support, support-ticket, help, lead, leads, demo, demo-request — match on the button/form `slug`, `id`, `label`, OR `submits_to` matching `/api/(contact|contact-us|contactus|get-in-touch|newsletter|subscribe|subscription|waitlist|wait-list|feedback|support|support-ticket|help|leads?|demo|demo-request)\b/`. Wider suppression biases toward NOT emitting a placeholder users entity on borderline public-form apps; the Tier-1 open question is still your signal that the synthesis agent was unsure.
>
...
>
> **Optional body fields.** Each `request_body` value is either a string type shorthand (`"title": "string"` — required by default) or an object (`"cover_image": {"type": "string", "required": false}`). Only `required: false` opts a field out — absent or `true` both mean required. Use the object form for any form input whose inventory entry has `validation.required: false`.
>
